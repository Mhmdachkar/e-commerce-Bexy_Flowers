...
const Ultra3DBouquetBuilder = () => {
  ...
  useEffect(() => {
    ...
    if (section && canvas) {
      ...
      const initialFlowers = [
        { id: 1, color: "#FFD700", position: [0, 0, 0] as [number, number, number] },
        { id: 2, color: "#FFFFFF", position: [0.4, 0.2, 0.2] as [number, number, number] },
        { id: 3, color: "#FFD700", position: [-0.3, 0.1, -0.2] as [number, number, number] }
      ];
  ...
  const resetBouquet = () => {
    const initialFlowers = [
      { id: Date.now(), color: "#FFD700", position: [0, 0, 0] as [number, number, number] }
    ];
    setFlowers(initialFlowers);
    setTotalPrice(25);
  };
